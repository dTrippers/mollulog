...
export default function CopyField({ text, label, className }: CopyFieldProps) {
  ...
  return (
    ...
    >
      ...
      <span className="text-xs text-neutral-400 opacity-0 transition-opacity group-hover:opacity-100">복사</span>
      <Icon
        className={`size-4 shrink-0 ${copied ? "text-green-500" : "text-neutral-400 group-hover:text-neutral-700 dark:group-hover:text-neutral-200"}`}
        strokeWidth={2}
      />
    </button>
  );
