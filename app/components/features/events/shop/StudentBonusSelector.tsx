...
export const StudentBonusSelector = memo(function StudentBonusSelector({
  ...
  const eventBonusStudents = useMemo(() => {
    return eventRewardBonus.flatMap(({ rewardBonuses }) => rewardBonuses.map(({ student }) => ({ uid: student.uid, name: student.name })))
  ...
  return (
    ...
    >
      ...
      <Toggle
        label="학생 이름 표시"
        initialState={showStudentName}
        onChange={setShowStudentName}
      />
      ...
      {tab === "student" && (
        <>
          <StudentCards mobileGrid={6} pcGrid={12} students={studentCardsData.map((student) => ({ ...student, name: showStudentName ? student.name : undefined }))} onSelect={actions.toggleBonusStudent} />
