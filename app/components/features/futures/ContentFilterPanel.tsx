...
export default function ContentFilterPanel({ filter, onFilterChange }: ContentFilterPanelProps) {
  ...
  const eventFilterProps = [
    {
      text: "메인 이벤트",
      active: filter.types.some((type) => ["event", "immortal_event", "fes", "collab"].includes(type)),
      onToggle: (activated: boolean) => onToggleType(activated, ["event", "immortal_event", "fes", "collab"]),
    },
    {
      text: "미니 이벤트",
      active: filter.types.includes("mini_event"),
      onToggle: (activated: boolean) => onToggleType(activated, ["mini_event"]),
    },
    {
      text: "스토리",
      active: filter.types.some((type) => ["main_story", "mini_story"].includes(type)),
      onToggle: (activated: boolean) => onToggleType(activated, ["main_story", "mini_story"]),
    },
    {
      text: "캠페인",
      active: filter.types.includes("campaign"),
      onToggle: (activated: boolean) => onToggleType(activated, ["campaign"]),
    },
    {
      text: "종합전술시험",
      active: filter.types.includes("joint_firing_drill"),
      onToggle: (activated: boolean) => onToggleType(activated, ["joint_firing_drill"]),
    },
    {
      text: "픽업 모집",
  ...
  const contentFilterProps = [
    {
      text: "총력전",
      active: filter.types.includes("total_assault"),
      onToggle: (activated: boolean) => onToggleType(activated, ["total_assault"]),
    },
    {
      text: "대결전",
      active: filter.types.includes("elimination"),
      onToggle: (activated: boolean) => onToggleType(activated, ["elimination"]),
    },
    {
      text: "제약해제결전",
      active: filter.types.includes("unlimit"),
      onToggle: (activated: boolean) => onToggleType(activated, ["unlimit"]),
    },
    {
      text: "연합작전",
      active: filter.types.includes("allied"),
      onToggle: (activated: boolean) => onToggleType(activated, ["allied"]),
    },
  ];
