...
export { isStudentStateMaintenanceResult };

export function StudentStateMaintenanceToast({ trigger }: { trigger: unknown }) {
  const [visible, setVisible] = useState(false);

  useEffect(() => {
    if (!isStudentStateMaintenanceResult(trigger)) {
      setVisible(false);
      return;
    }
