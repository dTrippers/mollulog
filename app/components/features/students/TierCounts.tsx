...
const colors: {
  [tier: number]: "red" | "orange" | "yellow" | "green" | "cyan" | "blue" | "purple" | "fuchsia" | "pink";
} = {
  1: "red",
  2: "orange",
  3: "yellow",
...
export default function TierCounts({
  tierCounts,
  visibleTiers,
  reducePaddings,
      ...
      <div className="grow flex flex-col">
        {visibleTiers.map((tier) => (
          <div key={`tier-ratio-${tier}`} className={`${reducePaddings ? "py-0" : "py-1"} flex-1`}>
            <Progress
              ratio={totalCount > 0 ? Math.min(1, (tierCounts[tier] ?? 0) / totalCount) : 0}
              color={colors[tier]}
            />
          </div>
        ))}
