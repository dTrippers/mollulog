...
export function MiniButton({ text, onClick, color = "default" }: MiniButtonProps) {
  let colorClass = {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`px-2.5 py-1 text-xs font-medium rounded-md transition whitespace-nowrap border cursor-pointer ${colorClass}`}
    >
      {text}
    </button>
  );
