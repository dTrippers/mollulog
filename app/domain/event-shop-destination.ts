...
function isOngoing(candidate: EventShopDestinationCandidate, now: UtcIsoString): boolean {
  ...
  const untilTime = candidate.until === null ? Number.POSITIVE_INFINITY : getInstantTime(candidate.until);
...
export function selectEventShopDestination(
  ...
  const ongoing = candidates.filter((candidate) => isOngoing(candidate, now)).sort(compareCandidates);
  if (ongoing.length > 0) {
    return ongoing[0];
  }
