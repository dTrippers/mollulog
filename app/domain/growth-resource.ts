...
export function getSkillMaterialChoiceBoxUid(kindOrder: number, rarity: number): string | null {
  if (kindOrder === GROWTH_RESOURCE_KIND_ORDER.bd) {
    return (
      SKILL_MATERIAL_CHOICE_BOX_UIDS_BY_KIND.bd[rarity as keyof typeof SKILL_MATERIAL_CHOICE_BOX_UIDS_BY_KIND.bd] ??
      null
    );
  }
...
export function calculateSkillResourceItems(
...
export type AbilityReleaseRequirements = {
  items: GrowthResourceItem[];
  credit: number;
  unavailable: boolean;
};
...
export function calculateAbilityReleaseRequirements(
  ...
  for (const { current, target, wbUid } of [
    {
      current: student.abilityHp,
      target: student.targetAbilityHp,
      wbUid: TARGET_ABILITY_RELEASE_WB_UIDS.targetAbilityHp,
    },
    {
      current: student.abilityAtk,
      target: student.targetAbilityAtk,
      wbUid: TARGET_ABILITY_RELEASE_WB_UIDS.targetAbilityAtk,
    },
    {
      current: student.abilityHeal,
      target: student.targetAbilityHeal,
      wbUid: TARGET_ABILITY_RELEASE_WB_UIDS.targetAbilityHeal,
    },
  ]) {
...
export function needsSkillResources(student: GrowthResourceStudentInput): boolean {
  return (
