...
export const action = async ({ context, request, params }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (job?.status !== "review_ready") throw new Error("검토할 수 있는 인식 결과가 없어요");
    ...
    if (existing) throw new Error("이미 처리 중인 인식 결과예요");
    ...
    for (const item of submittedItems) {
      if (item.candidateSelection) {
        ...
        if (!evidence) throw new Error("해당 셀의 인식 후보가 아닌 아이템은 반영할 수 없어요");
        candidateEvidenceByUid.set(item.itemUid, evidence);
      } else if (!resultItemsByUid.has(item.itemUid)) {
        throw new Error("인식 결과에 없는 아이템은 반영할 수 없어요");
      }
    }
    ...
    if (changedItems.length === 0) throw new Error("변경된 수량이 없어요");
    ...
    return data({ error: error instanceof Error ? error.message : "인식 결과를 반영하지 못했어요" }, { status: 400 });
  }
};

function parseApplyItems(value: unknown): ApplyItem[] {
  if (!value || typeof value !== "object" || !Array.isArray((value as { items?: unknown }).items)) {
    throw new Error("반영할 아이템을 선택해주세요");
  }
  const items = (value as { items: unknown[] }).items.map(parseApplyItem);
  if (items.length === 0) throw new Error("반영할 아이템을 선택해주세요");
  if (new Set(items.map((item) => item.itemUid)).size !== items.length) {
    throw new Error("같은 아이템이 두 번 포함되어 있어요");
  }
  return items;
}

function parseApplyItem(value: unknown): ApplyItem {
  if (!value || typeof value !== "object") throw new Error("반영할 아이템을 확인해주세요");
...
function parseCandidateSelection(value: unknown): CandidateSelection | undefined {
  ...
  if (!value || typeof value !== "object") throw new Error("아이템 후보 선택 정보를 확인해주세요");
  ...
  if (!imageFilename || !observationId) throw new Error("아이템 후보 선택 정보를 확인해주세요");
