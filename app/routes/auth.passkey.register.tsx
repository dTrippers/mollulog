...
export const loader = async ({ context, request }: LoaderFunctionArgs) => {
  ...
  const maintenance = await identityMaintenanceActionResult(env, { ctx, operation: "auth.passkey.register.options" });
...
export const action = async ({ context, request }: ActionFunctionArgs) => {
  const { env, ctx } = context.cloudflare;
  const maintenance = await identityMaintenanceActionResult(env, { ctx, operation: "auth.passkey.register" });
  if (maintenance) return maintenance;
  const currentUser = await getActiveSensei(env, request, ctx);
  if (!currentUser) {
    return redirect("/unauthorized");
