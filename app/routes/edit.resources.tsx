...
export const loader = async ({ context, request }: LoaderFunctionArgs) => {
  ...
  const [catalogResources, ownedQuantities, growthPlannerData] = await Promise.all([
    getItemCatalogResources(env),
    getUserResourceInventoryMap(env, currentUser.id),
    loadGrowthPlannerData(env, currentUser.id),
  ]);
  const requiredResources = aggregateGrowthResourceRequirements(
    growthPlannerData.managedStudents.map((student) => student.resourceRequirements),
  );
...
export default function EditResourcesPage() {
  const { resources, requiredResources, ownedQuantities } = useLoaderData<typeof loader>();
