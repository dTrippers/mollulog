...
export class Sha256 {
  ...
  update(input: Uint8Array): this {
    if (this.finalized) throw new Error("파일 정보 계산을 다시 시작해 주세요");
    this.byteLength += input.byteLength;
    let offset = 0;
    while (offset < input.byteLength) {
      const length = Math.min(64 - this.pendingLength, input.byteLength - offset);
  ...
  hexDigest(): string {
    if (this.finalized) throw new Error("파일 정보를 다시 계산해 주세요");
    this.finalized = true;
    ...
    view.setUint32(finalLength - 8, bitLengthHigh);
    view.setUint32(finalLength - 4, bitLengthLow);
...
export async function sha256File(file: File, onProgress?: (processedBytes: number) => void): Promise<string> {
